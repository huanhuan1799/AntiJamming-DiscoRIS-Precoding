%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This is the main file for testing DIRS-based P-FPJ and the effect of anti-jamming precoding against P-FPJ
...
sumRateZF_Anti_P12_Frame = zeros(length(ite_Num),Niter);
sumRateZF_Anti_P14_Frame = zeros(length(ite_Num),Niter);

sumRateZF_Anti_P12 = zeros(1,Niter);
sumRateZF_Anti_P14 = zeros(1,Niter);

sumRateZF_PFPJ_P12 = zeros(1,Niter);
sumRateZF_PFPJ_P14 = zeros(1,Niter);

%% Generate Random Reflecting Vector
clear Omg
b = 1; % quantization number
Amp = [0.8, 1]; % quantized amplitude of DIRS
Omg = [pi/9,7*pi/6]; % quantized phase shift of DIRS
aaa = zeros(1,Niter);
bbb = zeros(1,Niter);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
for m = 1:Niter
...
%% Generation Pha during the RPT and DT phases for P-FPJ
clear W User_Sig InterUser_Inter
% --------------------- RPT phase (case 1) --------------------- %
betaRPT_P14 = zeros(1,NN); % amplitude of IRS at RPT phase
phaDIRSRPT_P14 = zeros(1,NN); % phase shift of IRS at RPT phase
for ri = 1:length(phaDIRSRPT_P14)
    iddele = randi(2^b,1,1)*randi(2^b,1,1);
    if iddele == 1 % 1/4 probability (case 1)
        phaDIRSRPT_P14(ri) = Omg(1);
        betaRPT_P14(ri) = Amp(1); % phase-dependent amplitude
    else % 3/4 probability (case 1)
        phaDIRSRPT_P14(ri) = Omg(2);
        betaRPT_P14(ri) = Amp(2);
    end
end
theta_inbarRPT_P14 = betaRPT_P14.*exp(1j.*(phaDIRSRPT_P14)); % random reflecting vector at RPT phase
HRPT_P14 = Hr*diag(theta_inbarRPT_P14)*Gr + Hd; % equivalent channel
W_P14 = (HRPT_P14'/(HRPT_P14*HRPT_P14')) ; % ZF precoding
% -------------------------------------------------------------- %

% --------------------- RPT phase (case 2) --------------------- %
betaRPT_P12 = zeros(1,NN);
phaDIRSRPT_P12 = randi(2^b,1,NN);
for ri = 1:length(phaDIRSRPT_P12)
    if phaDIRSRPT_P12(ri) == 1 % 1/2 probability (case 2)
        betaRPT_P12(ri) = Amp(1); % phase-dependent amplitude
    else % 1/2 probability (case 2)
        betaRPT_P12(ri) = Amp(2); 
    end
end
theta_inbarRPT_P12 = betaRPT_P12.*exp(1j.*Omg(phaDIRSRPT_P12)); % random reflecting vector at RPT phase
HRPT_P12 = Hr*diag(theta_inbarRPT_P12)*Gr + Hd; % equivalent channel
...
sumRateZF_PFPJ_P14_1 = zeros(1,Tc);

theta_inbarDT_P12 = zeros(Tc,NN);
sumRateZF_Anti_P12_1 = zeros(1,Tc);
sumRateZF_PFPJ_P12_1 = zeros(1,Tc);
for ic = 1:Tc
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Stacha_Acc_P14 = lr*lg*NN* 1.2059 *(10.^(-noise/10)); % ¦Á^{_} = 1.2059 case 1 
Stacha_Acc_P12 = lr*lg*NN* 1.6078 *(10.^(-noise/10)); % ¦Á^{_} = 1.6078 case 2
...
for k = 1:K
    ...
    for u = 1:length(User_set)
        HD_age_The_P14 = HD_age_The_P14 + HRPT_P14(User_set(u),:)'*HRPT_P14(User_set(u),:)+Stacha_Acc_P14(User_set(u))*eye(N);
        HD_age_P14 = HD_age_P14 + HD_DT_EachTime_P14( User_set(u),:)'*HD_DT_EachTime_P14( User_set(u),:);
    end
    Ak_The_P14 = inv( HD_age_The_P14 + (1/P) *eye(N) ) * ...
        ( (HRPT_P14(k,:))'*HRPT_P14(k,:) +  Stacha_Acc_P14(k)*eye(N) ); % A_k case 1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
%% Persistent Fully Passive Jammer, case 1: 1/4
sumRateZF_PFPJ_P14_11 = 0;  
...
for k = 1:K
    ...
    sumRateZF_PFPJ_P14_11 = sumRateZF_PFPJ_P14_11 + log2( 1+User_Sig/(InterUser_Inter) ); % sum rate of PFPJ
    clear InterUser_Inter User_Sig
end  
sumRateZF_PFPJ_P14_1(ic) = sumRateZF_PFPJ_P14_11;

%% Persistent Fully Passive Jammer, case 2: 1/2
 sumRateZF_PFPJ_P12_11 = 0;  
...
for k = 1:K
    ...
    sumRateZF_PFPJ_P12_11 = sumRateZF_PFPJ_P12_11 + log2( 1+User_Sig/(InterUser_Inter) ); % sum rate of PFPJ
    clear InterUser_Inter User_Sig
end  
sumRateZF_PFPJ_P12_1(ic) = sumRateZF_PFPJ_P12_11;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
end
sumRateZF_Anti_P14(m) = sum(sumRateZF_Anti_P14_1)/Tc;
sumRateZF_Anti_P12(m) = sum(sumRateZF_Anti_P12_1)/Tc;

sumRateZF_PFPJ_P14(m) = sum(sumRateZF_PFPJ_P14_1)/Tc;
sumRateZF_PFPJ_P12(m) = sum(sumRateZF_PFPJ_P12_1)/Tc;
...
for it = 1:length(ite_Num)
    ...
    for is = 1:CurrMax_Ite
        ...
        %% Proposed Anti-Jamming in This Trans w/ the s-th est vua
        W_Anti_P14 = zeros(N,K);
        W_Anti_P12 = zeros(N,K);
        clear HD_DT_EachTime_P14 HD_DT_EachTime_P12
        %HD_Aging_EachTime = Hr*diag( theta_inbarDT_P14(ic,:)-theta_inbarRPT_P14 )*Gr + Hd;
    ...
    sumRateZF_Anti_P14_Frame(it,m) = sum(sumRateZF_Anti_P14_temp1)/Tc; % time-average
    sumRateZF_Anti_P12_Frame(it,m) = sum(sumRateZF_Anti_P12_temp1)/Tc;
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
plot(ite_Num,(sum(sumRateZF_Anti_P14)/K/Niter)*ones(1,length(ite_Num)) ,'r-.v','LineWidth',1.8,'MarkerSize',8); hold on 
plot(ite_Num,mean(sumRateZF_Anti_P14_Frame(:,:),2)/K,'cp','LineWidth',1.8,'MarkerSize',8);hold on 
plot(ite_Num,(sum(sumRateZF_Anti_P12)/K/Niter)*ones(1,length(ite_Num)) ,'b-.o','LineWidth',1.8,'MarkerSize',8);hold on 
plot(ite_Num,mean(sumRateZF_Anti_P12_Frame(:,:),2)/K,'c<','LineWidth',1.8,'MarkerSize',8);hold on 

plot(ite_Num,(sum(sumRateZF_PFPJ_P14)/K/Niter)*ones(1,length(ite_Num)) ,'r--s','LineWidth',1.8,'MarkerSize',8);hold on 
plot(ite_Num,(sum(sumRateZF_PFPJ_P12)/K/Niter)*ones(1,length(ite_Num)) ,'b--d','LineWidth',1.8,'MarkerSize',8);hold on 
...
% legend('W/O Jamming','Proposed AJP & C1','Proposed Est & C1','W/ PFPJ & C1',...
%     'Proposed AJP & C2','Proposed Est & C2','W/ PFPJ & C2','W/ {P_J} = -4 dBm','Location','Best');

yyaxis right
clear yy_temp yyy_temp yyyy_temp zz_temp zzz_temp zzzz_temp
yy_temp = mean(sumRateZF_Anti_P14_Frame(:,:),2)/K;
yyy_temp = (sum(sumRateZF_PFPJ_P14)/K/Niter)*ones(1,length(ite_Num));
yyyy_temp = ((yy_temp.' - yyy_temp)./yyy_temp)*100;

zz_temp = mean(sumRateZF_Anti_P12_Frame(:,:),2)/K;
zzz_temp = (sum(sumRateZF_PFPJ_P12)/K/Niter)*ones(1,length(ite_Num));
...
legend('W/O Jamming','Proposed AJP & C1','Proposed Est & C1',...
    'Proposed AJP & C2','Proposed Est & C2','W/ P-FPJ & C1','W/ P-FPJ & C2','W/ {P_J} = -4 dBm',...
